...
/**
 La aplicación debe implementar las operaciones CRUD
 ...
 * Gestión de Rutas:
    ...
    Los usuarios deben poder visualizar la lista de rutas disponibles. mostrar todas las rutas 
    Los usuarios deben poder buscar rutas por origen o destino. buscarOrigen/ buscarDestino 
...
public class RutaData {
    ...
    public void guardarRuta(Ruta ruta){
        String sql= "INSERT INTO `rutas`(`origen`, `destino`, `duracion_estimado`) VALUES (?,?,?)";
        ...
        try {
          ...
          JOptionPane.showMessageDialog(null, "Error al acceder a l tabla Ruta");
        }
    ...
    public List<Ruta>listarRuta(int idHorario){
        ...
        try {
            ...
            while(rs.next()){
                ...
                rut.setDuracion_estimado(rs.getTime("duracion_estimado").toLocalTime());
                rutas.add(rut);
            }
    ...
    public List<Ruta>buscarOrigen(String origen ){
     ArrayList<Ruta>rutas=new ArrayList<>();
     String sql="SELEC * FROM rutas WHERE origen=?";
        try {
            PreparedStatement ps= con.prepareStatement(sql);
            ps.setString(1, origen);
            ResultSet rs=  ps.executeQuery();
           ...
           JOptionPane.showMessageDialog(null, "Error al acceder a la tabla origen");
        }
        return rutas;
    }
    ...
    public List<Ruta>buscarDestino(String destino){
     ArrayList<Ruta>rutas=new ArrayList<>();
     String sql="SELEC * FROM rutas WHERE destino=?";
        try {
            PreparedStatement ps= con.prepareStatement(sql);
